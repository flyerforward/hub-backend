...
echo "[boot] entrypoint v8.2 (single-admin, stateless settings, KISS Admin-UI 404) loaded"

# ---- hard-fail required envs (stderr) ----
fail() { echo "[ERROR] $*" >&2; exit 64; }  # EX_USAGE
[ -n "${PB_ADMIN_EMAIL:-}" ]    || fail "PB_ADMIN_EMAIL must be set"
[ -n "${PB_ADMIN_PASSWORD:-}" ] || fail "PB_ADMIN_PASSWORD must be set"
...
list_admin_emails() { sql "SELECT email FROM _admins;"; }
cli_admin_delete() {
  /app/pocketbase $ENCRYPTION_ARG --dir /pb_data --migrationsDir /pb_migrations \
    admin delete "$1" >/tmp/pb_admin_delete.log 2>&1 || true
}
cli_admin_create() {
  /app/pocketbase $ENCRYPTION_ARG --dir /pb_data --migrationsDir /pb_migrations \
    admin create "$1" "$2" >/tmp/pb_admin_create.log 2>&1 || true
}

EXISTING_COUNT="$(sql "SELECT COUNT(*) FROM _admins;" 2>/dev/null || echo 0)"
EXISTING_LIST="$(list_admin_emails 2>/dev/null || true)"

echo "[bootstrap] Starting temp PB for admin check…"
start_temp

ADMIN_TOKEN="$(auth_token "$PB_ADMIN_EMAIL" "$PB_ADMIN_PASSWORD" || true)"

if [ -n "$ADMIN_TOKEN" ]; then
  echo "[admin] Env admin credentials are valid."
  stop_temp

  # prune any other admins
  if [ "${EXISTING_COUNT:-0}" -gt 0 ]; then
    echo "$EXISTING_LIST" | while IFS= read -r E; do
      [ -z "$E" ] && continue
      if [ "$E" != "$PB_ADMIN_EMAIL" ]; then
        echo "[admin] Deleting extra admin: $E"
        cli_admin_delete "$E"
      fi
    done
  fi
  ...
  if ! sql "SELECT 1 FROM _admins WHERE email='$(printf "%s" "$PB_ADMIN_EMAIL" | sed "s/'/''/g")' LIMIT 1;" | grep -q 1; then
    echo "[admin] Creating env admin (was missing)."
    cli_admin_create "$PB_ADMIN_EMAIL" "$PB_ADMIN_PASSWORD"
  fi

  start_temp
  ADMIN_TOKEN="$(auth_token "$PB_ADMIN_EMAIL" "$PB_ADMIN_PASSWORD" || true)"
  if [ -z "$ADMIN_TOKEN" ]; then
    echo "[ERROR] failed to re-auth after admin pruning."
    tail -n 200 /tmp/pb_bootstrap.log || true
    stop_temp
    exit 1
  fi
else
  echo "[admin] Env admin login failed. Replacing existing admins with env admin…"
  stop_temp
  if [ "${EXISTING_COUNT:-0}" -gt 0 ]; then
    echo "$EXISTING_LIST" | while IFS= read -r E; do
      [ -z "$E" ] && continue
      echo "[admin] Deleting admin: $E"
      cli_admin_delete "$E"
    done
  fi
  ...
  ADMIN_TOKEN="$(auth_token "$PB_ADMIN_EMAIL" "$PB_ADMIN_PASSWORD" || true)"
  if [ -z "$ADMIN_TOKEN" ]; then
    echo "[ERROR] could not authenticate newly created env admin."
    tail -n 200 /tmp/pb_bootstrap.log || true
    stop_temp
    exit 1
  fi
...
echo "[admin] Single-admin policy enforced."

# ---- stateless settings via API ----
