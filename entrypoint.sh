...
echo "[boot] entrypoint v8.0 (single-admin, stateless settings, admin-UI 404 hook) loaded"

############################################
# Env (no admin UI required)
############################################
...
# Required admin env (enforce single admin)
: "${PB_ADMIN_EMAIL:?PB_ADMIN_EMAIL must be set}"
: "${PB_ADMIN_PASSWORD:?PB_ADMIN_PASSWORD must be set}"

# Toggle for blocking Admin UI SPA (default true)
PB_BLOCK_ADMIN_UI="${PB_BLOCK_ADMIN_UI:-true}"
...
# sync repo migrations (if baked into image)
...
############################################
# Install Admin UI 404 hook (programmatic)
############################################
if [ "$PB_BLOCK_ADMIN_UI" = "true" ]; then
  cat >/app/pb_hooks/disable_admin_ui.pb.js <<'EOF'
/// <reference path="../pb_data/types.d.ts" />
// Auto-generated by entrypoint: 404 all Admin UI SPA routes in this environment.
onBeforeServe(({ router }) => {
  router.use("/_/*", (c, next) => {
    const m = String(c?.request?.method || "").toUpperCase();
    if (m === "GET" || m === "HEAD") return c.text("Not found", 404);
    return next();
  });
});
EOF
  echo "[hooks] Admin UI disabled via /app/pb_hooks/disable_admin_ui.pb.js"
else
  rm -f /app/pb_hooks/disable_admin_ui.pb.js 2>/dev/null || true
  echo "[hooks] Admin UI hook skipped (PB_BLOCK_ADMIN_UI=false)"
fi
...
if [ -n "$ADMIN_TOKEN" ]; then
  ...
  # Remove any admin with different email
  ...
  # Ensure target admin exists (in case DB was empty)
  ...
  # Re-auth for fresh token
  start_temp
  ADMIN_TOKEN="$(auth_token "$PB_ADMIN_EMAIL" "$PB_ADMIN_PASSWORD" || true)"
  if [ -z "$ADMIN_TOKEN" ]; then
    echo "[admin] ERROR: failed to re-auth after admin pruning."
  ...
  # Create the single desired admin
  echo "[admin] Creating env admin: $PB_ADMIN_EMAIL"
  cli_admin_create "$PB_ADMIN_EMAIL" "$PB_ADMIN_PASSWORD"

  # Start temp PB and auth
  start_temp
  ADMIN_TOKEN="$(auth_token "$PB_ADMIN_EMAIL" "$PB_ADMIN_PASSWORD" || true)"
  if [ -z "$ADMIN_TOKEN" ]; then
    echo "[admin] ERROR: could not authenticate newly created env admin."
    tail -n 200 /tmp/pb_bootstrap.log || true
    stop_temp
    exit 1
  fi
